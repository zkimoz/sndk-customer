...
const T = {
  ar: {
    ...
    navHome:'الرئيسية', navServices:'الخدمات', navContact:'تواصل معنا وتعرف علينا', navMore:'المزيد', navProfile:'حسابي',
    bookNow:'احجز الآن',
    switchLang:'EN',
  ...
  en: {
    ...
    navHome:'Home', navServices:'Services', navContact:'Contact & About Us', navMore:'More', navProfile:'My Account',
    bookNow:'Book Now',
    switchLang:'عربي',
...
// car.car_type is saved as whichever language the brand was picked in — look
// it up against either name column to get the English name carBrandLogoUrl needs.
const carLogoUrlFor = (car, carBrands = []) => {
  const match = carBrands.find(b => b.name_ar === car?.car_type || b.name_en === car?.car_type);
  return carBrandLogoUrl(match?.name_en || car?.car_type);
};
...
function openQuotationPDF(order, linked, profile, jobCard, carBrands = []) {
  ...
  <div class="page">
    ...
    <div class="footer">
      ...
      <div style="margin-top:10px;padding-top:10px;border-top:1px solid #e2e8f0;font-size:10px;color:#94a3b8;line-height:1.8">
        ...
        <div style="margin-top:2px">سجل تجاري C.R. : 244788</div>
        <div>رخصة تجارية Commercial License : 336567</div>
        <div>رقم قيد المنشأة Establishment Card : 17-3166-21</div>
      </div>
    </div>
  </div>
...
function ContactView({ isRtl }) {
  ...
  return (
    <div className="max-w-lg mx-auto p-4 md:p-6 space-y-5">
      <h1 className="text-2xl font-black text-center" style={{ color:C.text }}>{isRtl ? 'تواصل معنا وتعرف علينا' : 'Contact & About Us'}</h1>

      {introText && <p className="text-lg font-bold leading-relaxed text-center" style={{ color:C.text }}>{introText}</p>}
                    ...
                    <div key={a.id} ref={isHighlighted ? highlightRef : null} className="rounded-2xl overflow-hidden"
                      ...
                      {relOrd?.sent_to_customer ? (
                        <div className="p-4 space-y-3">
                          ...
                          {/* PDF */}
                          <button onClick={() => openQuotationPDF(relOrd, a, profile, jc, carBrandsRef)}
...
function HistoryOrderDetail({ jobCard, order, car, appt, profile, isRtl, tr, openVideoId, setOpenVideoId, carBrands, carCategories }) {
  ...
  return (
    <div className="px-4 pb-4 space-y-3" style={{ borderTop:`1px solid ${C.border}` }}>
      ...
      <div className="flex gap-2">
        <button onClick={() => openQuotationPDF(order || {}, linked, profile, jobCard, carBrands)}
      ...
      <div className="absolute bottom-0 end-0 -mb-3 -me-3 pointer-events-none">
        {service.ar === 'صيانة دورية' ? (
          ...
          <img src="/accident.png" alt="" aria-hidden="true"
            width={span===2?170:140} className="md:!w-[190px]" style={{ opacity:0.6, transform:'rotate(-6deg)', filter:'drop-shadow(6px 10px 8px rgba(0,0,0,0.45))' }}/>
        ) : (service.ar === 'تجديد استمارة السيارة' || service.en === 'Car Registration Renewal') ? (
          <img src="/istimara-qatar.jpg" alt="" aria-hidden="true"
            width={span===2?170:140} className="md:!w-[190px]" style={{ opacity:0.6, transform:'rotate(-16deg)', filter:'drop-shadow(6px 10px 8px rgba(0,0,0,0.45))' }}/>
